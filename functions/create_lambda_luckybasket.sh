...
aws ecr create-repository --repository-name aws_bedrock_repo --region us-west-1 \
...
aws lambda create-function --function-name aws_bedrock_lambda --package-type Image \
  --code ImageUri=039612858415.dkr.ecr.us-west-1.amazonaws.com/aws_bedrock_repo:latest \
